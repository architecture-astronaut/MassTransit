namespace MassTransit.RedisIntegration.Configurators
{
    ...
    public class RedisStreamReceiveEndpointConfiguration<TKey, TValue> :
        ReceiverConfiguration,
        ReceiveSettings,
        IRedisStreamReceiveEndpointConfigurator<TKey, TValue>
        where TValue : class
    {
        ...
        public RedisStreamReceiveEndpointConfiguration(IRedisHostConfiguration hostConfiguration, ConfigurationOptions consumerConfig, string Stream,
        ...
        public void ConfigureFetch(Action<IRedisFetchConfigurator> configure)
        {
            var configurator = new RedisFetchConfigurator(_consumerConfig);
        ...
        public void CreateIfMissing(Action<RedisStreamOptions> configure)
        {
            if (_options.TryGetOptions<RedisStreamOptions>(out _))
                throw new InvalidOperationException("Stream options has been already configured.");

            var options = new RedisStreamOptions(Stream);
        ...
        public override IEnumerable<ValidationResult> Validate()
        {
            ...
            if (_options.TryGetOptions(out RedisStreamOptions options))
            {
        ...
        public ReceiveEndpoint Build()
        {
            ...
            IRedisReceiveEndpointContext<TKey, TValue> CreateContext()
            {
                var builder = new RedisReceiveEndpointBuilder<TKey, TValue>(_busInstance, _endpointConfiguration, _hostConfiguration, this,
            ...
            IRedisReceiveEndpointContext<TKey, TValue> context = CreateContext();

            if (_options.TryGetOptions(out RedisStreamOptions options))
                _consumerConfigurator.UseFilter(new ConfigureTopologyFilter<TKey, TValue>(_hostConfiguration.Configuration, options));

            _consumerConfigurator.UseFilter(new RedisConsumerFilter<TKey, TValue>(context));
